...
;; (multiformats/dag-cbor), so the cljs build tests against the exact
;; pinned versions, not a hand-duplicated list that can drift. Jar entries
;; (Clojure/core.specs/spec.alpha on the JVM classpath) are filtered out;
;; shadow-cljs :source-paths wants directories only.
